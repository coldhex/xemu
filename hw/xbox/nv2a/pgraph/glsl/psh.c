...
static MString* psh_convert(struct PixelShader *ps)
{
    ...
    if (ps->state.depth_clipping) {
        if (ps->state.z_perspective) {
            mstring_append(clip, "float zvalue = 1.0/gl_FragCoord.w + depthOffset;\n"
             ...
             * clipping. TODO: is there a better way?
             */
            mstring_append(clip, "if ((gl_FragCoord.z + 1.0f/16777216.0f)*clipRange.y < clipRange.z ||\n"
    ...
    if (ps->state.z_perspective) {
        if (!ps->state.depth_clipping) {
            mstring_append(ps->code, "float zvalue = 1.0/gl_FragCoord.w + depthOffset;\n");
        }

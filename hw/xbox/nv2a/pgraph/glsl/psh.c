...
static MString* psh_convert(struct PixelShader *ps)
{
    ...
    if (ps->state.z_perspective) {
        mstring_append(
            ...
            "if (zvalue > 0.0) {\n"
            "  zvalue += depthOffset;\n"
            "  zvalue += depthFactor*triMZ*zvalue*zvalue;\n"
            "} else {\n"
    ...
    case DEPTH_FORMAT_F16:
        ...
        mstring_append(
            ps->code,
            "float zvaluef = clamp(zvalue, clipRange.z, clipRange.w);\n"
            "uint zvaluei = floatBitsToUint(max(zvaluef, 0.0)) >> 11;\n"
            "zvaluei = clamp(zvaluei, 0x78000u, 0x87000u) - 0x78000u;\n"
    ...
    case DEPTH_FORMAT_F24:
        ...
        mstring_append(
            ...
            "gl_FragDepth = uintBitsToFloat(floatBitsToUint(zvaluef / 16777216.0) + 1u);\n");
        break;
    case DEPTH_FORMAT_D16:
        // 16-bit unsigned int
        mstring_append(
            ps->code,
            "gl_FragDepth = floor(clamp(floor(zvalue), clipRange.z, clipRange.w)) / 65535.0;\n");
        break;
    ...
    default:
        ...
        mstring_append(
            ps->code,
            "float zvaluef = floor(clamp(floor(zvalue), clipRange.z, clipRange.w));\n"
            "gl_FragDepth = uintBitsToFloat(floatBitsToUint(zvaluef / 16777216.0) + 1u);\n");
        break;
    }
